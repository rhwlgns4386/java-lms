...
public class SessionRegisterInfoTest {
    ...
    void 유료_강의_학생_수가_0명_이하일_시_예외_발생() {
        Assertions.assertThatThrownBy(() ->
                        Session.createPaidSession(0L, image, SessionType.PAID, SessionStatus.READY, 1L, 0, sessionDuration, SessionRegisteringStatus.OPEN))
    ...
    void 모집중인_강의_수강신청_테스트() {
        ...
        session.register(NsUserTest.JAVAJIGI, 100L,SelectStatus.SELECTED);
    ...
    void 모집중이지_않은_강의_수강신청_불가(SessionStatus status) {
        Session session = Session.createPaidSession(0L, image, SessionType.PAID, status, 200L, 1, sessionDuration, SessionRegisteringStatus.CLOSED);

        Assertions.assertThatThrownBy(
                        () -> session.registerStudent(NsUserTest.JAVAJIGI, 200L,SelectStatus.SELECTED)
                ).isInstanceOf(IllegalArgumentException.class)
                .hasMessage("이 강의는 모집중인 상태가 아닙니다");
    }
    ...
    void 결제정보_저장_확인() {
        ...
        session.register(NsUserTest.JAVAJIGI, 200L,SelectStatus.SELECTED);
