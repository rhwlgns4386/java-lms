...
public class SessionInfoTest {
    ...
    void 무료_강의_생성() {
        Assertions.assertThatCode(() ->
                Session.createFreeSession(0L, image, SessionType.FREE, SessionStatus.READY, sessionDuration, SessionRegisteringStatus.OPEN));
    }
    ...
    void 무료_강의_가격_0원_인지_확인() {
        Assertions.assertThat(
                        Session.createFreeSession(0L, image, SessionType.FREE, SessionStatus.READY, sessionDuration, SessionRegisteringStatus.OPEN).getPrice())
                .isEqualTo(0);
    }
    ...
    void 유료_강의_가격이_0원_이하일_시_예외_발생() {
        Assertions.assertThatThrownBy(() ->
                        Session.createPaidSession(0L, image, SessionType.PAID, SessionStatus.READY, 0L, 1, sessionDuration, SessionRegisteringStatus.OPEN))
    ...
    void 수강료_일치하지_않을_경우_예외_발생(int testCase) {
        Session session = Session.createPaidSession(0L, image, SessionType.PAID, SessionStatus.REGISTER, 200L, 1,
                sessionDuration, SessionRegisteringStatus.OPEN);

        Assertions.assertThatThrownBy(
                        () -> session.register(NsUserTest.JAVAJIGI, testCase,SelectStatus.SELECTED)
                ).isInstanceOf(IllegalArgumentException.class)
    ...
    void 유료강의_수강인원_최대일_때_수강신청하면_예외_발생() {
        ...
        session.registerStudent(NsUserTest.JAVAJIGI, 200L, SelectStatus.SELECTED);

        Assertions.assertThatThrownBy(
                        () -> session.register(NsUserTest.JAVAJIGI, 200,SelectStatus.SELECTED)
                ).isInstanceOf(IllegalArgumentException.class)
